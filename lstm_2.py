...
class SLSTM:
  def __init__(self, alpha_size, lstm_size, lstm_depth, sess):
    ...
    self.sess = sess
    self.lstm_state_size = self.lstm_size * self.lstm_depth
    self.def_input()
    self.def_params()
    self.def_model()
    self.def_output()
  ...
  def def_input(self,):
    ...
    with tf.name_scope('input'):
      ...
      self.init_state = tf.placeholder(tf.float32,
        [None, self.lstm_size*self.lstm_depth], name='init_state')
  ...
  def def_params(self):
    ...
    with tf.name_scope('params'):
      # LSTM
      self.lstm_layers = [rnn.GRUCell(self.lstm_size) for _ in range(self.lstm_depth)]
      # FC
  ...
  def def_model(self):
    ...
    with tf.name_scope('model'):
      # chains LSTM layers
      self.dropcells = [rnn.DropoutWrapper(cell,input_keep_prob=.7) for cell in self.lstm_layers]
      self.lstm = rnn.MultiRNNCell(self.dropcells, state_is_tuple=False)
      self.lstm = rnn.DropoutWrapper(self.lstm, output_keep_prob=.7)
      # unrolls LSTM layers
